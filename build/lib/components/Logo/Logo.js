 ...
 */
function setFillInline(pathTag, color) {
    if (!color) {
 ...
 */
function applyAvonInlineRules({ svg, hexColor, specialFill }) {
    // Pega cor original de um path se não formos sobrescrever
    function getOriginalFill(pathStr) {
 ...
 */
function applyStyleSubstitutions({ svg, hexColor }) {
 ...
 */
function processSvg({ svg, hexColor, specialFill, width, height }) {
...
function getLogoType(color) {
    return color === 'neutral' ? 'neutral' : 'custom';
}
function getCustomColor(color, theme) {
    return color === 'neutral' ? undefined : getColorByName(theme, color);
}
function getLogoByProps(color, model, theme) {
...
export const Logo = ({ accessibilityLabel, color = 'neutral', model = 'a', size = 'veryHuge', languages = 'default' }) => {
    const theme = useTheme();
    const { file, height, width, es, pt } = getLogoByProps(color, model, theme);
    ...
    return (<Container>
      <SvgCss accessible={!!accessibilityLabel} accessibilityLabel={accessibilityLabel} accessibilityRole="image" testID="logo" xml={processedSvg}/>
    </Container>);
};
